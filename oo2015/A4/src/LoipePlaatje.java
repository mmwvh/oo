...
public class LoipePlaatje extends JFrame implements TekenLoipe {
	/**
	 * Used for painting the tiles. We assume that all tiles have same width and
	 * height.
	 */
	 ...
	 * 
	 * @param s
	 *            this Loipe contains the 2D track information
	 */
	...
	/**
	 * Checks validity of the input track description. Terminates normally when
	 * the track is valid, prints a message otherwise.
	 */
	 ...
	 * 
	 * @param naam
	 *            "name.png" is filename of picture
	 * @return the picture
	...
	private ImageIcon plaatje(String name) {
		...
		if (resource != null) {
			...
			System.err.printf(
					"ERROR: Resource \"%s\" not found... aborting...\n", name);
			System.err
					.println("For NetBeans the file should be placed in the src folder.");
	...
	public void paint(Graphics g) {
		...
		if (previousPos != null) {
			...
			g.fillOval(previousPos.getX() * UNIT + UNIT / 4, previousPos.getY()
	...
	/**
	 * The method to call in order to draw a window with the given track. You
	 * are supposed to use this method once for each object.
	 */
	@Override
	public void teken() {
		if (loipeOK) {
			...
			System.err
					.println("LoipePlaatje kan niet tekenen. Loipe is ongeldig of plaatjes niet gevonden");
	}
	...
	public void setPosition(Punt p) {
		...
		if (previousPos != null) {
			g.setColor(SNOW);
			g.fillRect(previousPos.getX() * UNIT, previousPos.getY() * UNIT,
					UNIT, UNIT);
			tekenFragment(g, previousPos.getX(), previousPos.getY());
		}
		g.setColor(Color.RED);
		g.fillOval(p.getX() * UNIT + UNIT / 4, p.getY() * UNIT + UNIT / 4,
				UNIT / 2, UNIT / 2);
		previousPos = p;
	}
}