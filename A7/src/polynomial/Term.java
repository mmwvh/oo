...
public class Term implements Comparable<Term> {
	...
	public String toString() {
		if (coefficient == 0.000000) {
			return String.format("0", exponent, coefficient);
		} else if (exponent == 0) {
			return String.format("%f", coefficient, exponent);
		} else if (exponent == 1) {
			return String.format("%fx", coefficient, exponent);
		} else if (coefficient == 1.000000) {
			return String.format("x^%d", exponent, coefficient);
		} else if (coefficient == -1.000000) {
			return String.format("-x^%d", exponent, coefficient);
		} else {
	 ...
	 *
	 * @param s
	 *            the scanner providing the input
